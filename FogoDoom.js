...
function calculateFirePropagation() {
    for (let column = 0; column < fireWidth; column++) {
        for (let row = 0; row < fireHeigth; row++) {
            ...
            updateFireIntensityPropagation(pixelIndex);
        }
    }
    renderFire();
}

function updateFireIntensityPropagation(currentPixelIndex) {
...
function renderFire() {
    ...
    for (let row = 0; row < fireHeigth; row++) {
        ...
        for (let column = 0; column < fireWidth; column++) {
            const pixelIndex = column + (fireWidth * row);
            const firePixelIntesity = firePixelsArray[pixelIndex];

            if (debug === true) {
                html += '<td>';
                html += `<div class="pixel-index">${pixelIndex}</div>`;
                html += firePixelIntesity;
                html += '</td>';
            }
            else {
                const color = fireColorPallet[firePixelIntesity];
